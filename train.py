...
parser.add_argument('--corpus_type', type=str, default="military", help="NYT,military")
...
def train(model, data_iterator, optimizer, params, ex_params):
    ...
    loss_avg_cl = utils.RunningAverage()
    loss_avg_com = utils.RunningAverage()
    ...
    for step, _ in enumerate(t):
        ...
        batch1 = tuple(t.to(params.device) for t in batch[:-2])
        input_ids,attention_mask,seq_tags,potential_rel,major_rel_tags,minor_rel_tags= batch1
        potential_major, potential_minor = batch[-2], batch[-1]
        # compute model output and loss
        loss, loss_seq, loss_rel, loss_cl, loss_com= model(input_ids,
                                                           attention_mask=attention_mask,
                                                           seq_tags=seq_tags,
                                                           major_rel_tags=major_rel_tags,
                                                           minor_rel_tags=minor_rel_tags,
                                                           potential_rel=potential_rel,
                                                           major_triples = potential_major,
                                                           minor_triples = potential_minor,
                                                           ex_params=ex_params)
        ...
        loss_avg_cl.update(loss_cl.item())
        loss_avg_com.update(loss_com.item())
        ...
        t.set_postfix(loss='{:05.3f}'.format(loss_avg()),
                      ...
                      loss_cl='{:05.3f}'.format(loss_avg_cl()),
                      loss_com='{:05.3f}'.format(loss_avg_com()))
...
def train_and_evaluate(model, params, ex_params, restore_file=None):
    ...
    for epoch in range(1, args.epoch_num + 1):
        ...
        major_metrics, minor_metrics, nary_metrics, _, _,_, _, _,_ = \
            evaluate(model, val_loader, params, ex_params, mark='Val')
        val_f1 = nary_metrics['f1']
